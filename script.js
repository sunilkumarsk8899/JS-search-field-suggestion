...
function handleSearchFunctionality(){
    ...
    input.addEventListener('input',function(){
        // if(input.value != ''){
            var clutter = '';
            var arrayFilterData = arr.filter((obj) => obj.name.toLowerCase().startsWith(input.value.toLowerCase()));

            document.querySelector('.searchdata').style.display = 'block';
            arrayFilterData.forEach(function(obj){
...
function searchClear(){
    var input = document.querySelector('#searchinput');
    var clearBtn = document.querySelector('.search_clear');
